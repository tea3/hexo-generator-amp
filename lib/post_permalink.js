...
function postPermalinkFilter(data, inConfig) {
  ...
  if (!permalink || permalink.rule !== (config.generator_amp.generateAmpPath || ( "amp/" + config.permalink) ) ) {
    permalink = new Permalink( ( config.generator_amp.generateAmpPath || ("amp/" + config.permalink) ) );
  }
