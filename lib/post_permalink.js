...
function postPermalinkFilter(data, inConfig) {
  var config = inConfig;
  var meta = {
    id: data.id || data._id,
    title: data.slug,
  ...
  if (!permalink || permalink.rule !== (config.generator_amp.generateAmpPath || (config.permalink + "/amp/") ) ) {
    permalink = new Permalink( ( config.generator_amp.generateAmpPath || (config.permalink + "/amp/") ) );
  }
